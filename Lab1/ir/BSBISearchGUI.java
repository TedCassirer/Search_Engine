...
public class BSBISearchGUI extends JFrame {

    /**  The indexer creating the search index. */
    BSBIIndexer indexer = new BSBIIndexer();
    ...
    public static final String homeDir = "~/Desktop/Skolan/Lab1/";
    //public static final String wikiDir = "~/Desktop/Skolan/Lab1/out/production/Lab1/davisWiki";
    ...
    JMenuItem loadIndexItem = new JMenuItem( "Load index" );
    ...
    private void createGUI() {
        ...
        Action search = new AbstractAction() {
            public void actionPerformed( ActionEvent e ) {
                ...
                synchronized ( indexLock ) {
                    results = indexer.bsbi.search( query, queryType, rankingType, structureType );
                }
                ...
                if ( results != null ) {
                    ...
                    for ( int i=0; i<results.size(); i++ ) {
                        buf.append( " " + i + ". " );
                        String filename = indexer.bsbi.docIDs.get( "" + results.get(i).docID );
                        if ( filename == null ) {
                            buf.append( "" + results.get(i).docID );
                        }
                        else {
                            filename = filename.substring(44, filename.length()-2);
                            buf.append( filename );
                        }
        ...
        Action saveAndQuit = new AbstractAction() {
            public void actionPerformed( ActionEvent e ) {
                ...
                System.out.println(indexer.indexTree);
                indexer.bsbi.cleanup();
                System.exit( 0 );
            }
        };
        saveItem.addActionListener( saveAndQuit );

        Action index = new AbstractAction() {
            public void actionPerformed( ActionEvent e ) {
                index();
            }
        };
        indexItem.addActionListener( index );
        ...
        loadIndexItem.addActionListener( loadIndex );
    ...
    private void index() {
        ...
        synchronized ( indexLock ) {
            ...
            for ( int i=0; i<dirNames.size(); i++ ) {
                ...
                System.out.println("Indexing files in " + dirNames);
    ...
    public void loadIndex(){
        String path = indexer.indexStoragePath + "tree.ser";
        System.out.println(path);
        try{
            FileInputStream fis = new FileInputStream(path);
            ObjectInputStream ois = new ObjectInputStream(fis);
            TreeMap<Integer, String> indexTree = (TreeMap<Integer, String>) ois.readObject();
            indexer.addIndexTree(indexTree);
            ois.close();
        ...
        System.out.println(indexer.indexCount + " indexes loaded from " + path);
    ...
    public static void main( String[] args ) {
        BSBISearchGUI s = new BSBISearchGUI();
        s.createGUI();
        s.decodeArgs( args );
        //s.index();
        s.loadIndex();
